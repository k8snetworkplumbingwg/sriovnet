...
func TestGetUplinkRepresentorWithPhysPortName(t *testing.T) {
	tcases := []struct {
	...
	}{
		{
			...
			vfReps:               []*repContext{{"enp_0", "pf0vf0", "111111"}, {"enp_1", "pf0vf1", "111111"}},
...
func TestGetVfRepresentorDPU(t *testing.T) {
	tcases := []struct {
		...
		{
			name:          "Host VFs only",
			vfReps:        []*repContext{{"eth0", "c1pf0vf0", "c2cfc60003a1420c"}, {"eth1", "c1pf0vf1", "c2cfc60003a1420c"}, {"eth2", "c1pf0vf2", "c2cfc60003a1420c"}},
			pfID:          "0",
			vfID:          "2",
			expectedVFRep: "eth2",
			shouldFail:    false,
		},
		{
			...
			vfReps:        []*repContext{{"eth0", "c1pf0vf0", "c2cfc60003a1420c"}, {"eth1", "pf0vf0", "c2cfc60003a1420c"}, {"eth2", "pf0vf2", "c2cfc60003a1420c"}, {"eth3", "c1pf0vf2", "c2cfc60003a1420c"}},
			pfID:          "0",
			vfID:          "2",
			expectedVFRep: "eth3",
			shouldFail:    false,
		},
		{
			...
			vfReps:        []*repContext{{"eth0", "pf0vf0", "c2cfc60003a1420c"}, {"eth1", "pf0vf1", "c2cfc60003a1420c"}, {"eth2", "pf0vf2", "c2cfc60003a1420c"}},
			pfID:          "0",
			vfID:          "2",
			expectedVFRep: "eth2",
			shouldFail:    false,
...
func TestGetSfRepresentor(t *testing.T) {
	tcases := []struct {
	...
	}{
		{
			name:          "Local SFs only",
			uplink:        "p0",
			sfReps:        []*repContext{{"eth0", "pf0sf0", "c2cfc60003a1420c"}, {"eth1", "pf0sf1", "c2cfc60003a1420c"}, {"eth2", "pf0sf2", "c2cfc60003a1420c"}},
			expectedSFRep: "eth2",
			sfIndex:       2,
			shouldFail:    false,
		},
		{
			...
			uplink:        "p0",
			sfReps:        []*repContext{{"eth0", "pf0sf0", "c2cfc60003a1420c"}, {"eth1", "pf0sf1", "c2cfc60003a1420c"}, {"eth2", "c1pf0sf2", "c2cfc60003a1420c"}, {"eth3", "pf0sf2", "c2cfc60003a1420c"}},
			expectedSFRep: "eth3",
			sfIndex:       2,
			shouldFail:    false,
		},
		{
			...
			uplink:        "p0",
			sfReps:        []*repContext{{"eth0", "pf0sf0", "c2cfc60003a1420c"}, {"eth1", "pf0sf1", "c2cfc60003a1420c"}, {"eth2", "c1pf0sf2", "c2cfc60003a1420c"}},
			expectedSFRep: "",
			sfIndex:       2,
			shouldFail:    true,
		},
		...
		},
		{
			...
			uplink:        "p0",
			sfReps:        []*repContext{{"eth0", "c1pf0sf0", "c2cfc60003a1420c"}, {"eth1", "c1pf0sf1", "c2cfc60003a1420c"}, {"eth2", "c1pf0sf2", "c2cfc60003a1420c"}},
			expectedSFRep: "",
			sfIndex:       2,
			shouldFail:    true,
		},
		{
			...
			uplink:        "p0",
			sfReps:        []*repContext{{"eth0", "c1pf0sf0", "c2cfc60003a1420c"}, {"eth1", "pf0sf1", "c2cfc60003a1420c"}},
			expectedSFRep: "",
			sfIndex:       3,
			shouldFail:    true,
		},
		{
			...
			uplink:        "p1",
			sfReps:        []*repContext{{"eth0", "c1pf0sf3", "c2cfc60003a1420c"}, {"eth1", "pf0sf3", "c2cfc60003a1420c"}},
			expectedSFRep: "",
			sfIndex:       3,
			shouldFail:    true,
		},
...
func TestGetPortIndexFromRepresentor(t *testing.T) {
	tcases := []struct {
	...
	}{
		{
			name:          "VF rep",
			netdev:        "eth5",
			reps:          []*repContext{{"eth5", "pf0vf5", "c2cfc60003a1420c"}},
			expectedID:    5,
			shouldFail:    false,
			expectedError: "",
		},
		{
			name:          "SF rep",
			netdev:        "eth5",
			reps:          []*repContext{{"eth5", "pf0sf5", "c2cfc60003a1420c"}},
			expectedID:    5,
			shouldFail:    false,
			expectedError: "",
		},
		{
			...
			netdev:        "eth5",
			reps:          []*repContext{{"eth5", "c1pf0vf5", "c2cfc60003a1420c"}},
			expectedID:    5,
			shouldFail:    false,
			expectedError: "",
		},
		{
			name:          "externalSF rep",
			netdev:        "eth5",
			reps:          []*repContext{{"eth5", "c1pf0sf5", "c2cfc60003a1420c"}},
			expectedID:    5,
			shouldFail:    false,
			expectedError: "",
		},
		{
			...
			netdev:        "pf0hpf",
			reps:          []*repContext{{"pf0hpf", "pf0", "c2cfc60003a1420c"}},
			expectedID:    0,
			shouldFail:    true,
		...
		},
		{
			...
			netdev:        "p0",
			reps:          []*repContext{{"p0", "p0", "c2cfc60003a1420c"}},
			expectedID:    0,
			shouldFail:    true,
			expectedError: "unsupported port flavor",
		},
		{
			...
			netdev:        "eth5",
			reps:          []*repContext{{"eth5", "", "c2cfc60003a1420c"}},
			expectedID:    0,
			shouldFail:    true,
...
func TestGetSfRepresentorDPU(t *testing.T) {
	tcases := []struct {
		...
		{
			name:          "Host SFs only",
			vfReps:        []*repContext{{"eth0", "c1pf0sf1", "c2cfc60003a1420c"}, {"eth1", "c1pf0sf2", "c2cfc60003a1420c"}, {"eth2", "c1pf0sf3", "c2cfc60003a1420c"}},
			pfID:          "0",
			sfID:          "2",
			expectedSFRep: "eth1",
			shouldFail:    false,
		},
		{
			...
			vfReps:        []*repContext{{"eth0", "c1pf0sf0", "c2cfc60003a1420c"}, {"eth1", "pf0sf0", "c2cfc60003a1420c"}, {"eth2", "pf0sf2", "c2cfc60003a1420c"}, {"eth3", "c1pf0sf2", "c2cfc60003a1420c"}},
			pfID:          "0",
			sfID:          "2",
			expectedSFRep: "eth3",
			shouldFail:    false,
		},
		{
			...
			vfReps:        []*repContext{{"eth0", "pf0sf0", "c2cfc60003a1420c"}, {"eth1", "pf0sf1", "c2cfc60003a1420c"}, {"eth2", "pf0sf2", "c2cfc60003a1420c"}},
			pfID:          "0",
			sfID:          "2",
			expectedSFRep: "",
			shouldFail:    true,
		},
		{
			...
			vfReps:        []*repContext{{"eth0", "c1pf0sf0", "c2cfc60003a1420c"}},
			pfID:          "0",
			sfID:          "5",
			expectedSFRep: "",
			shouldFail:    true,
...
func TestGetVfRepresentorPortFlavour(t *testing.T) {
	tcases := []struct {
	...
	}{
		{
			...
			netdev:     "eth0",
			rep:        repContext{"eth0", "p0", "c2cfc60003a1420c"},
		...
		},
		{
			name:       "PF flavor",
			netdev:     "eth0",
			rep:        repContext{"eth0", "pf0", "c2cfc60003a1420c"},
		...
		},
		{
			name:       "VF flavor",
			netdev:     "eth0",
			rep:        repContext{"eth0", "pf0vf0", "c2cfc60003a1420c"},
		...
		},
		{
			...
			netdev:     "eth0",
			rep:        repContext{"eth0", "c1pf0vf0", "c2cfc60003a1420c"},
			expected:   PORT_FLAVOUR_PCI_VF,
			shouldFail: false,
		},
		{
			name:       "SF flavor",
			netdev:     "eth0",
			rep:        repContext{"eth0", "pf0sf0", "c2cfc60003a1420c"},
		...
		},
		{
			...
			netdev:     "eth0",
			rep:        repContext{"eth0", "c1pf0sf0", "c2cfc60003a1420c"},
			expected:   PORT_FLAVOUR_PCI_SF,
			shouldFail: false,
		},
		{
		...
		},
		{
			...
			netdev:     "eth0",
			rep:        repContext{"eth0", "", "c2cfc60003a1420c"},
			expected:   PORT_FLAVOUR_UNKNOWN,
			shouldFail: true,
		},
		{
			...
			netdev:     "eth0",
			rep:        repContext{"eth0", "invalid", "c2cfc60003a1420c"},
			expected:   PORT_FLAVOUR_UNKNOWN,
			shouldFail: false,
		},
		{
			...
			netdev:     "eth1",
			rep:        repContext{"eth0", "pf0vf34", "c2cfc60003a1420c"},
			expected:   PORT_FLAVOUR_UNKNOWN,
			shouldFail: true,
		},
	}
...
func TestSetRepresentorPeerMacAddress(t *testing.T) {
	...
	tcases := []struct {
	...
	}{
		{
			...
			netdev:      "pf0vf24",
			reps:        []*repContext{{"pf0vf24", "c1pf0vf24", "c2cfc60003a1420c"}, {"p0", "p0", "c2cfc60003a1420c"}},
		...
		},
		{
			...
			netdev:      "pf0vf24",
			reps:        []*repContext{{"pf0vf24", "pf0vf24", "c2cfc60003a1420c"}, {"p0", "p0", "c2cfc60003a1420c"}},
			expectedMac: mac.String(),
			shouldFail:  false,
		},
		{
			...
			netdev:      "pf0hpf",
			reps:        []*repContext{{"pf0hpf", "pf0", "c2cfc60003a1420c"}, {"p0", "p0", "c2cfc60003a1420c"}},
			expectedMac: "",
			shouldFail:  true,
		},
		{
